...
async function setFrontmatterKey(app: App, file: TFile, key: string, value: string) {
	...
	if (hasFM) {
		...
		// Match spaces/tabs only. Using \s here also consumed the newline after an
		// empty property, causing values to be written on the following line.
		const keyRegex = new RegExp(`^(${key}:[\\t ]*)(.*)$`, "m");
		if (keyRegex.test(fmBlock)) {
			content = fmBlock.replace(keyRegex, `$1${value}`) + rest;
		} else {
			content = fmBlock + `\n${key}: ${value}` + rest;
		}
	} else {
